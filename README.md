...
- Streams real-time Audio Visualizer data (real FFT-based frequency analysis, not a fake amplitude sampler)
...
The app streams live audio analysis data, computed with a real FFT (not a time-domain approximation):

- Volume level (mono + separate Left/Right channel levels)
- Bass / Mid / Treble
- 16-band frequency spectrum array
...
```json
{
  ...
  "mid": 43,
  "treble": 40,
  "spectrum": [50, 36, 42, 40, 41, 38, 37, 42, 38, 41, 35, 30, 22, 15, 8, 2],
  "updatedAt": "21:18:20"
}
```
...
> All JSON fields use `camelCase`. Every message also carries `schemaVersion` (a protocol version number) and `appVersion` (the running app's version) — check `schemaVersion` if you're building an integration that needs to detect an outdated/mismatched client.
...
- One-click update from the About panel — downloads, verifies, and installs the new version for you
- Update status is echoed as a small banner when it's relevant

##### ✨ Quality of Life
- Click Title / Artist / Album → copy to clipboard
- Always on top (optional)
- Run at startup
- Start minimized to tray (no visible flash on launch)
- Dark theme
- Hide tray icon
...
Latest version: [Releases](https://github.com/mrgogo7/NowPlaying.WebSocket/releases/latest)
